...
class SimpleAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        match raw_path.split('/'):
            case ['', 'api', 'report'] if \
                    (after := queries.get('after', None)) and \
                    (before := queries.get('before', None)) and \
                    (feeds := queries.get('feeds')):

                if report := db.get_latest_report(after, before, feeds):
                    self.return_json({'text': report.text,
                                      'after': report.after.isoformat(),
                                      'before': report.before.isoformat(),
                                      'timestamp_generated': report.timestamp})
                else:
