...
def report(after, before, feeds):
    ...
    if len(to_report) > 0:
        report_query = f'''
        Combine all subsequent information, which is a series of docuuments \
                separated by a single line containing "=====" that each \
                summarize the key points from important information sources, \
                into a clear and informative report:\n\n \
                {(chr(10) + "=====" + chr(10)).join(to_report)}
        '''.strip()
        report_text = chatgpt(report_query)
    else:
        report_text = 'Nothing to report.'
    return db.insert_report(after, before, feed_list,
                            report_text, report_entries)
